...
<body>
<section id="MySongs" class="most-listened-container">
		<h1 class="section-title">My Songs</h1>
		
		<div class="searchbar">
            <form action="searchMorceau" method="POST">
                <input class="search" placeholder="Type here to find songs..."
		...
		<div class="best-products">
			...
			<c:forEach items="${dataSummary.listeMorceaux}" var="i">
				...
				<c:if test="${i.album.couverture!=null}">
				<img class="couverture" src="${pageContext.request.contextPath}/ressourcesweb/images/mes_albums/album${i.album.id}.png">
				</c:if>
					<c:url value="/lecture" var="ghi">
						<c:param name="id" value="${i.id}" />
					</c:url>
					<a href="${ghi}" class="middle image"></a>
					<c:url value="/modification" var="abc">
						<c:param name="id" value="${i.id}" />
					</c:url>
					<a href="${abc}" class="modify"></a>
					<div class="product-bio">
							<h1 class="product-name">${i.nomMorceau}</h1>
							<h3 class="product-amount">${i.artiste}</h3>
							<c:url value="/suppression" var="def">
								<c:param name="id" value="${i.id}" />
							</c:url>
							<a href="${def}" class="bin"></a>
						</div>
					</div>
				</c:forEach>
